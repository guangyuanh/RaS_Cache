...
def security_metric(plot_data, key_msb):
    ...
    # compute the pearson correlation for each cache block
...
if __name__ == '__main__':
    ...
    n_score_block = 16
    # first column uses only 6 entries to save the final result:
    # mean_coef, mean_p, min_coef, min_p, max_coef, max_p
    # following 32 columns are (coef, p) of 16 bytes
...
#        plt.locator_params(axis = 'y', nbins = 20)
        ...
        save_score[0,0] = save_score[0,0] + sec_scores[n_score_block, 0]/16
        save_score[1,0] = save_score[1,0] + sec_scores[n_score_block, 1]/16
